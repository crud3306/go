...
首先，两个有序的数组，分别是arr[begin,mid-1]，arr[mid,end]，此时初始化i=begin，j=mid，k=end，从i~j为左有序的数组，k~j为右有序的数组：

将i向后移动，找到第一个arr[i]>arr[j]的索引，这个时候，i前面的部分已经排好序了，begin~i这些元素已经是两个有序数组的前n小个元素。



然后将j向后移动，找到第一个arr[j]>arr[i]的索引：
